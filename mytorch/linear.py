...
class Linear():
    def __init__(self, in_feature, out_feature, weight_init_fn, bias_init_fn):
        ...
        self.dW = np.zeros(None)#np.zeros([len(self.W), len(self.W[0])])
        self.db = np.zeros(None)#np.zeros([len(self.b), len(self.b[0])])
    ...
    def backward(self, delta):
        ...
        """
        #print(delta.shape)
        self.dW = np.matmul(self.x.transpose(), delta) * (1/len(delta))
        self.db = delta.mean(axis=0).reshape(1,-1)

        #print(self.db)
        self.dx = np.matmul(self.W, delta.transpose())
        #self.dx = np.matmul(delta,self.W.transpose())
        return self.dx
