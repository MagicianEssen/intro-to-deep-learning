...
class Sigmoid(Activation):
    ...
    def derivative(self):
        ...
        return self.forward(self.store)*(1-self.forward(self.store))
        #raise NotImplemented
...
#
#     def derivative(self):
#         return 1-self.forward(self.store)**2
class Tanh(Activation):
    ...
    def forward(self, x):
        shape = x.shape
        self.state = np.tanh(x).reshape(shape)
        return self.state
...
class ReLU(Activation):
    ...
    def forward(self, x):
        ...
        self.store = np.copy(x)
        shape = x.shape
        x = np.maximum(x,0).reshape(shape)
        return x

    def derivative(self):
        return np.where(self.store<=0, 0.0, 1.0)
