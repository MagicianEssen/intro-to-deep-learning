...
class CNN(object):
    ...
    def __init__(self, input_width, num_input_channels, num_channels, kernel_sizes, strides,
        ...
        # <---------------------
        num_all_channels = [num_input_channels]+num_channels
        self.convolutional_layers = []
        templayer = input_width
        for i in range(self.nlayers):
            templayer = int(np.floor((templayer-kernel_sizes[i])/strides[i])+1)
            self.convolutional_layers.append(Conv1D(num_all_channels[i], num_all_channels[i+1], kernel_sizes[i], strides[i], conv_weight_init_fn, bias_init_fn))
        self.flatten = Flatten()
        templayer = num_all_channels[-1] * templayer
        self.linear_layer = Linear(templayer, num_linear_neurons, linear_weight_init_fn, bias_init_fn)
    ...
    def backward(self, labels):
        ...
        for i in range(len(self.convolutional_layers)):
            #print(grad.shape, self.activations[len(self.convolutional_layers)-i-1].derivative().shape )
            grad = self.activations[len(self.convolutional_layers)-i-1].derivative() * grad
            grad = self.convolutional_layers[len(self.convolutional_layers)-i-1].backward(grad)
