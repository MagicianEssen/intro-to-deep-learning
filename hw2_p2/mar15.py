...
train_loader_args = dict(shuffle=True, batch_size=64, num_workers=num_workers, pin_memory=True) if cuda  else dict(shuffle=True, batch_size=128)
train_loader = data.DataLoader(trainset, **train_loader_args)

val_loader_args = dict(shuffle=True, batch_size=64, num_workers=num_workers, pin_memory=True) if cuda  else dict(shuffle=True, batch_size=128)
...
def train_closs(model, data_loader, test_loader, task='Classification'):
    ...
    for epoch in range(numEpochs):
        avg_loss = 0.0
        pbar = tqdm(total=round(len(trainset)/64))
        scheduler_label.step(avg_loss)
        scheduler_closs.step(avg_loss)
        ...
        if task == 'Classification':
            ...
            #train_loss, train_acc = test_classify_closs(model, train_loader)
        ...
        if(val_acc>highest_acc): 
            #trainer.save_model('./network_model.pt')
            torch.save(network, 'mar13_network_model.pt')
            ...
            name = 'resnet18 with epoch: '+str(epoch+1)+'.csv'
...
network.to(device)
train_closs(network, train_loader, val_loader)
...
#df = pd.read_csv('./11-785hw2p2-s20/test_order_classification.txt', header=None, names=['Id'])